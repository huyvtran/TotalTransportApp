...
export class NoticeServiceProvider {
  ...
  //通知信息分页查询
  getSysNoticeListByPage(sysNotice, page, rows) {
    let url = AppConfig.getUrl() + '/app/sysNotice/getSysNoticeListByPage.do';
  ...
  //公告详情查询
  getSysNoticeById(id) {
    let url = AppConfig.getUrl() + '/app/sysNotice/getSysNoticeById.do';
    let params = {id: id};
    return new Promise((resolve, reject) => {
      this.http.post(url, this.toQueryString(params), this.options)
        .map(res => res.json())
